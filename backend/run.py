...
def process_video(video_path, stats_path=None, show_output=False):
    ...
    try:
        ...
        if stats_path is None:
            ...
            stats_path = os.path.join(STATS_DIR, f"{video_id}_stats.json")
        ...
        while True:
            ...
            # Players: process + draw if showing output
            players_for_stats = []
            ...
            # Update stats (passes/possession)
            evt = pass_counter.update(now_t, ball_xy, players_for_stats)
            if show_output and evt is not None:
                tteam = evt["team"]
            ...
            # Optional display elements when showing output
            if show_output:
                # Timer overlay
        ...
        print(f"[DONE] Final stats written to {stats_path}")
        return True
